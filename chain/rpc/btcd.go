...
type RPCClient struct {
	*rpcclient.Client
	...
	enqueueNotification chan chain.Notification
	dequeueNotification chan chain.Notification
...
var _ chain.Client = (*RPCClient)(nil)
...
func NewRPCClient(chainParams *chaincfg.Params, connect, user, pass string, certs []byte,
	...
	client := &RPCClient{
		...
		enqueueNotification: make(chan chain.Notification),
		dequeueNotification: make(chan chain.Notification),
...
func (c *RPCClient) Start() error {
	...
	// If the RPCClient has already run once, it can't be restarted.
	select {
	case <-c.quit:
		return errors.New("RPCClient cannot be restarted.")
	default:
	}
...
// later reads.
func (c *RPCClient) Notifications() <-chan chain.Notification {
